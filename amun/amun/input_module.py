...
def get_relative_time(data, dataset):
    ...
    data['relative_time'] = (data['time:timestamp'] - data['time:timestamp_2']).dt.total_seconds() / 60.0   # in  minutes
...
def annotate_state_per_case(data,dafsa_states):
    ...
    return data[['concept:name', 'activity_index', 'prev_state', 'state']]
...
def annotate_eventlog_with_state_vectorized(data,data_dir,dataset,trace_variants):
    ...
    trace_variants_df.drop(columns=['level_1'],axis=1, inplace=True)
